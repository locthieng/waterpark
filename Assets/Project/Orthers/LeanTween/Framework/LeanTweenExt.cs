...
{
    /// <summary>
    /// Move a game object through a set of points (NOT bezier nor curved line whatsoever).
    /// </summary>
    /// <param name="gameObject"></param>
    /// <param name="setOfPoints">First point will not be used in movement. First point must be the position of this very object.</param>
    /// <param name="time"></param>
    /// <param name="speed"></param>
    /// <param name="isLoop"></param>
    /// <returns></returns>
    public static LTDescr LeanMoveThroughPoints(this GameObject gameObject, Vector3[] setOfPoints, float time, float speed, float startDelay, bool isLoop, Action onCompelete = null)
    {
        return gameObject.MoveToPoint(setOfPoints, 1, time, speed, startDelay, isLoop, onCompelete).setSpeed(speed).setDelay(startDelay);
    }

    private static LTDescr MoveToPoint(this GameObject gameObject, Vector3[] setOfPoints, int pointIndex, float time, float speed, float startDelay, bool isLoop, Action onCompelete = null)
    {
        return gameObject.LeanMove(setOfPoints[pointIndex], time).setSpeed(speed).setOnComplete(() =>
        {
            pointIndex++;
            if (pointIndex < setOfPoints.Length)
            {
                gameObject.MoveToPoint(setOfPoints, pointIndex, time, speed, startDelay, isLoop, onCompelete);
            }
            else if (isLoop)
            {
                gameObject.transform.position = setOfPoints[0];
                onCompelete?.Invoke();
                gameObject.LeanMoveThroughPoints(setOfPoints, time, speed, startDelay, isLoop);
            }
            else
            {
                onCompelete?.Invoke();
    ...
    public static LTDescr LeanMoveLocalThroughPoints(this GameObject gameObject, Vector3[] setOfPoints, float time, float speed, float startDelay, bool isLoop, Action onCompelete = null)
    {
        return gameObject.MoveLocalToPoint(setOfPoints, 1, time, speed, startDelay, isLoop, onCompelete).setSpeed(speed).setDelay(startDelay);
    }

    private static LTDescr MoveLocalToPoint(this GameObject gameObject, Vector3[] setOfPoints, int pointIndex, float time, float speed, float startDelay, bool isLoop, Action onCompelete = null)
    {
        return gameObject.LeanMoveLocal(setOfPoints[pointIndex], time).setSpeed(speed).setOnComplete(() =>
        {
            pointIndex++;
            if (pointIndex < setOfPoints.Length)
            {
                gameObject.MoveLocalToPoint(setOfPoints, pointIndex, time, speed, startDelay, isLoop, onCompelete);
            }
            else if (isLoop)
            {
                gameObject.transform.position = setOfPoints[0];
                onCompelete?.Invoke();
                gameObject.LeanMoveLocalThroughPoints(setOfPoints, time, speed, startDelay, isLoop);
            }
            else
            {
                onCompelete?.Invoke();
